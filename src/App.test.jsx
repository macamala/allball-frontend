...
describe("NinkoSports Phase 3 routes", () => {
  ...
  it("does not expose Data Sources in public navigation", async () => {
    ...
    expect(screen.queryByRole("link", { name: /Powered by/i })).toBeNull();
    expect(document.body.textContent).not.toMatch(/SportScore|TheSportsDB/i);
    cleanup();
...
describe("Production quality surfaces", () => {
  ...
  it("renders connected live scores without leaking providers", async () => {
    ...
    expect(screen.queryByRole("link", { name: /Powered by/i })).toBeNull();
    expect(document.body.textContent).not.toMatch(/SportScore|sportscore:widget/i);
  ...
  it("shows finished, live and upcoming events together on Today ALL", async () => {
    ...
    expect(arsenalRow.querySelector(".score-pair-stack")?.textContent).toMatch(/2/);
    expect(arsenalRow.querySelector(".score-pair-stack")?.textContent).toMatch(/1/);
